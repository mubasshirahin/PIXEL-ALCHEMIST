...
void updateRope(){
	if (isThrowing) {
		ropeidx2 += 10; // Move the rope upwards
...
void showPani(){
	...
	if (charX > 180 && charX < 220 && charY >80 && charY < 120 && level3 && pullDone && !waterCollected){
...
void showGlass(){
	...
	if (charX > 800 && charX < 980 && charY >150 && charY < 220 && !pickedGlass && level3){
		iText(500, 10, "Enter X to Collect", GLUT_BITMAP_TIMES_ROMAN_24);
		nearGlass = true;
	}
}
...
void showToPickingEarth(){
	if (charX > 550 && charX<700 && charY>30 && charY < 100 && waterCollected && map2 && level3 && !pickingDirt2){
		iText(500, 10, "Enter X to Collect", GLUT_BITMAP_TIMES_ROMAN_24);
		nearDirt2 = true;
		earthCollected = true;
	}
...
void showDead(){
	if (lavaOn && !map3){
		iShowBMP2(200, 400, "Level3//dead.bmp", 0);
		iShowBMP2(300, 400, "Level3//dead.bmp", 0);
		iShowBMP2(700, 400, "Level3//dead.bmp", 0);
		iShowBMP2(800, 400, "Level3//dead.bmp", 0);
	}
}
